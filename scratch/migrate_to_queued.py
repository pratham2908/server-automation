...
async def migrate():
    ...
    vid_result = await db.videos.update_many(
        {"channel_id": {"$in": ig_channel_ids}, "status": "scheduled", "instagram_media_id": None},
        {"$set": {"status": "queued"}},
    )
    ...
    queue_ig = await db.schedule_queue.update_many(
        {"channel_id": {"$in": ig_channel_ids}}, {"$set": {"platform": "instagram"}}
    )
    ...
    queue_yt = await db.schedule_queue.update_many(
        {"channel_id": {"$in": yt_channel_ids}}, {"$set": {"platform": "youtube"}}
    )
