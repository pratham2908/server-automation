...
async def test_reach():
    ...
    client = AsyncIOMotorClient(settings.MONGODB_URI, tls=True, tlsContext=ssl_context)
    db = client[settings.MONGODB_DB_NAME]

    # 1. Find a channel with tokens
    channel = await db.channels.find_one(
        {"youtube_tokens.token": {"$exists": True}, "platform": "youtube"}
    )
    if not channel:
    ...
    video = await db.videos.find_one(
        {"channel_id": channel_id, "youtube_video_id": {"$exists": True}, "status": "published"}
    )
    if not video:
