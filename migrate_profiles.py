...
async def migrate():
    ...
    result = await db.channels.update_many(
        {"profile_id": {"$exists": False}}, {"$set": {"profile_id": profile_id}}
    )
    result2 = await db.channels.update_many(
        {"profile_id": "default"}, {"$set": {"profile_id": profile_id}}
    )

    print(
        f"Migrated {result.modified_count + result2.modified_count} channels to profile {profile_id}"
    )
